...
function HistoryMenu({ showHistoryMenu, setShowHistoryMenu }) {
		...
		<>
			<div
				className={`fixed z-20 w-full h-full md:w-[60%] md:h-[85%] flex justify-center items-center  md:ml-[20%]  ${
					showHistoryMenu ? "md:my-[3.8%] m-0" : "-mt-[500%]"
			...
			>
				...
				>
					<button className={`flex h-10 w-10 items-center justify-center border-solid rounded-lg cursor-pointer ml-8 mt-6 text-[30px] ${styleThemeColors.text}`} onClick={() => setShowHistoryMenu(false)}>
						<TfiClose />
					</button>
				<h1 className={`text-3xl text-center ${styleThemeColors.text}`}>Historial</h1>
...
HistoryMenu.propTypes = {
	showHistoryMenu: PropTypes.bool.isRequired,
	setShowHistoryMenu: PropTypes.func.isRequired,
};
